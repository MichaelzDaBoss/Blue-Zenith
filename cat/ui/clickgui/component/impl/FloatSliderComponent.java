...
public class FloatSliderComponent extends Component implements Visible {

    public FloatValue value;

    public FloatSliderComponent(FloatValue value) {
        this.value = value;
    }
    ...
    public void mouseClicked(double x, double y, int mouseX, int mouseY, int button) {
        ...
        if(mouseX >= x && mouseX <= x + width && mouseY >= y && mouseY <= y + height) {
            float inc = makePrecise(((double) mouseX - x) / ((x + width) - x), 4);
            float off = makePrecise(value.min + ((value.max - value.min) * inc), 3);
            value.setValuePrecise(makePrecise(off, 3));
        }
    ...
    public static float makePrecise(double value, int precision) {
        float pow = (float) Math.pow(10, precision);
