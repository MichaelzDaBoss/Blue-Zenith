...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                if(rt.getState() && entity == Minecraft.getMinecraft().thePlayer){
                    f = this.interpolateRotation(rt.prevYaw, rt.yaw, partialTicks);
                    f1 = this.interpolateRotation(rt.prevYaw, rt.yaw, partialTicks);
                    f7 = rt.prevPitch + (rt.pitch - rt.prevPitch) * partialTicks;
                }
                float f2 = f1 - f;
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        ...
        if (flag || flag1 || (chamsState && EntityManager.isTarget(entitylivingbaseIn, false))) {
    ...
    public void renderName(T entity, double x, double y, double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if(BlueZenith.moduleManager.getModule(NameTags.class).getState() && EntityManager.isTarget(entity, false) && Minecraft.getMinecraft().theWorld.loadedEntityList.contains(entity) && !(entity instanceof EntityArmorStand)){
                GlStateManager.disableLighting();
